...
sudo echo "listen=udp:MACHINE_IP:5060" >> /usr/local/etc/kamailio/kamailio.cfg
sudo echo "listen=tcp:MACHINE_IP:5060" >> /usr/local/etc/kamailio/kamailio.cfg
EOF
...
sudo sed -i 's/EXTRA_OPTS=""/EXTRA_OPTS="-l MACHINE_IP -d DBUG:LOG_LOCAL0"/' /etc/default/rtpproxy

sudo sed -i "s/MACHINE_IP/$MACHINE_IP/g" /etc/default/rtpproxy
...
sudo systemctl disable rtpproxy
sudo systemctl mask rtpproxy
...
echo "#################################################################################################################################################################################################"
#espeak "Installation and Configuration completed!"
...
echo "Open four new terminals for PCSCF, SCSCF, ICSCF and FHoSS and sudo su each of them. Use different terminals for each one"
echo "Run PCSCF:"
